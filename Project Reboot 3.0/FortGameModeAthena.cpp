...
// Define a struct to store the common data for all playlists
struct ServerEmbedData {
	std::string mode;
	std::string version; // Adding a version field to the struct
};

// Function to send the server embed JSON
void SendServerEmbed(const ServerEmbedData& data) {
...
// Function to handle the ServersUpEmbed based on the playlist name
void ServersUpEmbed() {
	if (PlaylistName == "/Game/Athena/Playlists/Playlist_DefaultSolo.Playlist_DefaultSolo") {
		ServerEmbedData data;
		data.mode = "Solos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	...
	else if (PlaylistName == "/Game/Athena/Playlists/Playground/Playlist_Playground.Playlist_Playground")
	{
		ServerEmbedData data;
	...
	else if (PlaylistName == "/Game/Athena/Playlists/Creative/Playlist_PlaygroundV2.Playlist_PlaygroundV2")
	{
		ServerEmbedData data;
		data.mode = "PlaygroundV2";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/Snipers/Playlist_Snipers_Solo.Playlist_Snipers_Solo")
	{
		ServerEmbedData data;
		data.mode = "Snipers Shootout Solos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/Snipers/Playlist_Snipers_Duos.Playlist_Snipers_Duos")
	{
		ServerEmbedData data;
		data.mode = "Snipers Shootout Duos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/HighExplosives/Playlist_HighExplosives_Solo.Playlist_HighExplosives_Solo")
	{
		ServerEmbedData data;
		data.mode = "High Explosives Solos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/HighExplosives/Playlist_HighExplosives_Squads.Playlist_HighExplosives_Squads")
	{
		ServerEmbedData data;
		data.mode = "High Explosives Squads";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/Fill/Playlist_Fill_Solo.Playlist_Fill_Solo")
	{
		ServerEmbedData data;
		data.mode = "Floor is lava Solos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/Fill/Playlist_Fill_Duos.Playlist_Fill_Duos")
	{
		ServerEmbedData data;
		data.mode = "Floor is lava Duos";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else if (PlaylistName == "/Game/Athena/Playlists/Fill/Playlist_Fill_Squads.Playlist_Fill_Squads")
	{
		ServerEmbedData data;
		data.mode = "Floor is lava Squads";
		data.version = std::format("{}", Fortnite_Version); // Assigning the version
		SendServerEmbed(data);
	}
	else 
